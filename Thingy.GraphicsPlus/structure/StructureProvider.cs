...
namespace Thingy.GraphicsPlus
{
    public class StructureProvider : IStructureProvider
    {
        ...
        public StructureProvider(IJointFactory jointFactory, IElementFactory elementFactory, ITriangleFactory triangleFactory)
        {
        ...
        public IJoint CreateFromStructure(string structure, IDictionary<string, string> structures)
        {
            ...
            while (!string.IsNullOrEmpty(line) || lines.Count > 0)
            {
                switch (line[0])
                {
                    ...
                    case 'T': CreateTriangle(lines, line, joints); break;
                }
        ...
        private void AddElementPoints(IElement element, string line)
        {
            ...
            foreach(string point in points)
            {
                ...
                element.AddPoint(new PointF(Convert.ToSingle(values[0]), Convert.ToSingle(values[1])));
            }
        }
